
# Wall sample count
scoreboard players set gen.wall.count config 35

# Floor sample count
scoreboard players set gen.floor.count config 4

# Wall sample count
scoreboard players set gen.brewing_stand.count config 36

### Decor

# Large Decor sample count
scoreboard players set gen.decor.large.count config 7

# Medium Decor sample count
scoreboard players set gen.decor.medium.count config 11

# Small decor sample count
scoreboard players set gen.decor.small.count config 16
