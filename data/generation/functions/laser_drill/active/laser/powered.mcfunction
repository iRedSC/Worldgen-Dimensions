...
execute if block ~ ~1 ~ air if block ~ ~2 ~ air run summon falling_block ~ ~1 ~ {BlockState:{Name:"minecraft:red_stained_glass_pane"},NoGravity:1b,Time:1,Tags:["laser"]}
execute if block ~ ~2 ~ air run summon falling_block ~ ~2 ~ {BlockState:{Name:"minecraft:red_stained_glass_pane"},NoGravity:1b,Time:1,Tags:["laser"]}
kill @e[tag=laser]
